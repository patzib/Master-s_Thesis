...
# Import configuration variables (e.g., file paths, model names) from a central config file.
# This practice enhances modularity and makes the system easier to configure and maintain.
import config
...
def run_preprocessing_pipeline():
    ...
    """
    logging.info(f"Executing external pre-processing script: {config.PREPROCESSING_SCRIPT_NAME}...")
    try:
        # Ensure the directory for processed documents exists before running the script.
        os.makedirs(config.PROCESSED_DOC_FOLDER, exist_ok=True)
        ...
        command = [
            python_executable, config.PREPROCESSING_SCRIPT_NAME,
            "--input_dir", str(config.RAW_DOC_FOLDER),
            "--output_dir", str(config.PROCESSED_DOC_FOLDER),
            "--chunk_size", str(config.CHUNK_SIZE),
            "--overlap_size", str(config.OVERLAP_SIZE),
        ]
        ...
        logging.error(f"Error: The script '{config.PREPROCESSING_SCRIPT_NAME}' was not found.")
        raise
...
def init_ollama_model():
    ...
    try:
        ...
        if config.MODEL_NAME not in model_names:
            logging.info(f"Model '{config.MODEL_NAME}' not found. Creating it now...")
            ...
            logging.info(f"Custom model '{config.MODEL_NAME}' created successfully.")
        else:
            logging.info(f"Custom model '{config.MODEL_NAME}' already exists.")
    except Exception as e:
        logging.error(f"Failed to initialize Ollama model: {e}")
        raise

def initialize_and_update_vector_db(embeddings: HuggingFaceEmbeddings) -> Chroma:
    """
    ...
    manifest_path = config.PERSIST_DIRECTORY / "ingested_files.log"
    ...
    if os.path.exists(config.PROCESSED_DOC_FOLDER):
        for filename in os.listdir(config.PROCESSED_DOC_FOLDER):
            if filename.startswith("chunks_") and filename.endswith(".json"):
                all_processed_files.add(os.path.join(config.PROCESSED_DOC_FOLDER, filename))
...
def setup_chatbot():
    ...
    vector_db = initialize_and_update_vector_db(embeddings)
    if not vector_db:
    ...
    base_retriever = vector_db.as_retriever(
        search_kwargs={"k": config.INITIAL_RETRIEVAL_K} # Retrieves the top K documents.
    )
    ...
    logging.info(f"Initializing re-ranker model: {config.RERANKER_MODEL}")
    cross_encoder_model = HuggingFaceCrossEncoder(model_name=config.RERANKER_MODEL)
    compressor = CrossEncoderReranker(model=cross_encoder_model, top_n=config.TOP_N_RERANKED)
    ...
    logging.info(f"Re-ranking retriever created. Will retrieve {config.INITIAL_RETRIEVAL_K} docs and keep the top {config.TOP_N_RERANKED}.")
