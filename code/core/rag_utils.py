...
This script contains utility functions that support the main RAG application.
Its primary function, `get_lecture_filter_from_query`, uses an LLM to perform
real-time analysis on user prompts to extract specific metadata, such as
lecture numbers. This enables the dynamic, query-driven filtering feature
...
# Use a relative import to access the config file from within the same package.
from . import config
...
def get_lecture_filter_from_query(question: str) -> int:
    ...
    """
    try:
        # The prompt is structured to instruct the LLM on its specific task:
